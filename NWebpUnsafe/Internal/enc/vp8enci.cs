...
namespace NWebp.Internal
{
	class vp8enci
	{
		...
		// version numbers
		const int ENC_MAJ_VERSION = 0;
		const int ENC_MIN_VERSION = 1;
		const int ENC_REV_VERSION = 3;

		// size of histogram used by CollectHistogram.
		const int MAX_COEFF_THRESH = 64;
		...
		//         +----+
		const int BPS       16   // this is the common stride
		const int Y_SIZE   (BPS * 16)
		const int UV_SIZE  (BPS * 8)
		const int YUV_SIZE (Y_SIZE + UV_SIZE)
		const int PRED_SIZE (6 * 16 * BPS + 12 * BPS)
		const int Y_OFF    (0)
		const int U_OFF    (Y_SIZE)
		const int V_OFF    (U_OFF + 8)
		const int ALIGN_CST 15
		const int DO_ALIGN(PTR) ((uintptr_t)((PTR) + ALIGN_CST) & ~ALIGN_CST)
		...
		// intra 16x16
		const int I16DC16 (0 * 16 * BPS)
		const int I16TM16 (1 * 16 * BPS)
		const int I16VE16 (2 * 16 * BPS)
		const int I16HE16 (3 * 16 * BPS)
		// chroma 8x8, two U/V blocks side by side (hence: 16x8 each)
		const int C8DC8 (4 * 16 * BPS)
		const int C8TM8 (4 * 16 * BPS + 8 * BPS)
		const int C8VE8 (5 * 16 * BPS)
		const int C8HE8 (5 * 16 * BPS + 8 * BPS)
		// intra 4x4
		const int I4DC4 (6 * 16 * BPS +  0)
		const int I4TM4 (6 * 16 * BPS +  4)
		const int I4VE4 (6 * 16 * BPS +  8)
		const int I4HE4 (6 * 16 * BPS + 12)
		const int I4RD4 (6 * 16 * BPS + 4 * BPS +  0)
		const int I4VR4 (6 * 16 * BPS + 4 * BPS +  4)
		const int I4LD4 (6 * 16 * BPS + 4 * BPS +  8)
		const int I4VL4 (6 * 16 * BPS + 4 * BPS + 12)
		const int I4HD4 (6 * 16 * BPS + 8 * BPS +  0)
		const int I4HU4 (6 * 16 * BPS + 8 * BPS +  4)
		const int I4TMP (6 * 16 * BPS + 8 * BPS +  8)

		typedef long score_t;     // type used for scores, rate, distortion
		const int MAX_COST ((score_t)0x7fffffffffffffLL)

		const int QFIX 17
		const int BIAS(b)  ((b) << (QFIX - 8))
		...
		typedef uint proba_t;   // 16b + 16b
		typedef byte ProbaArray[NUM_CTX][NUM_PROBAS];
		typedef proba_t StatsArray[NUM_CTX][NUM_PROBAS];
		typedef ushort CostArray[NUM_CTX][MAX_VARIABLE_LEVEL + 1];
		...
		typedef struct {
		  byte segments_[3];     // probabilities for segment tree
		  byte skip_proba_;      // final probability of being skipped.
		...
		typedef struct {
		  ...
		  byte alpha_;      // quantization-susceptibility
		} VP8MBInfo;

		typedef struct VP8Matrix {
		  ushort q_[16];        // quantizer steps
		  ushort iq_[16];       // reciprocals, fixed point.
		  ushort bias_[16];     // rounding bias
		  ushort zthresh_[16];  // value under which a coefficient is zeroed
		  ushort sharpen_[16];  // frequency boosters for slight sharpening
		} VP8Matrix;
		...
		typedef struct {
		  ...
		  short y_dc_levels[16];    // Quantized levels for luma-DC, luma-AC, chroma.
		  short y_ac_levels[16][16];
		  short uv_levels[4 + 4][16];
		  int mode_i16;               // mode number for intra16 prediction
		  byte modes_i4[16];       // mode numbers for intra4 predictions
		  int mode_uv;                // mode number of chroma prediction
		  uint nz;                // non-zero blocks
		} VP8ModeScore;
		...
		typedef struct {
		  ...
		  byte*      yuv_in_;           // borrowed from enc_ (for now)
		  byte*      yuv_out_;          // ''
		  byte*      yuv_out2_;         // ''
		  byte*      yuv_p_;            // ''
		  ...
		  byte*      preds_;            // intra mode predictors (4x4 blocks)
		  uint*     nz_;               // non-zero pattern
		  byte       i4_boundary_[37];  // 32+5 boundary samples needed by intra4x4
		  byte*      i4_top_;           // pointer to the current top boundary sample
		...
		const int MAX_NUM_TOKEN = 2048;

		typedef struct VP8Tokens VP8Tokens;
		struct VP8Tokens {
		  ushort tokens_[MAX_NUM_TOKEN];  // bit#15: bit, bits 0..14: slot
		  int left_;
		  VP8Tokens* next_;
		};
		...
		struct VP8Encoder {
		  ...
		  byte*   preds_;     // predictions modes: (4*mb_w+1) * (4*mb_h+1)
		  uint*  nz_;        // non-zero bit context: mb_w+1
		  byte*   yuv_in_;    // input samples
		  byte*   yuv_out_;   // output samples
		  byte*   yuv_out2_;  // secondary scratch out-buffer. swapped with yuv_out_.
		  byte*   yuv_p_;     // scratch buffer for prediction
		  byte   *y_top_;     // top luma samples.
		  byte   *uv_top_;    // top u/v samples.
								 // U and V are packed into 16 pixels (8 U + 8 V)
		  byte   *y_left_;    // left luma samples (adressable from index -1 to 15).

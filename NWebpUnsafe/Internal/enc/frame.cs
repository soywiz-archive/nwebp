...
namespace NWebp.Internal
{
	class frame
	{
		...
		const int SEGMENT_VISU = 0;
		const int DEBUG_SEARCH = 0;    // useful to track search convergence
		...
		const int SKIP_PROBA_THRESHOLD = 250;  // value below which using skip_proba is OK.
		...
		void TOKEN_ID(b, ctx, p) { return ((p) + NUM_PROBAS * ((ctx) + (b) * NUM_CTX)); }
		...
		#if SEGMENT_VISU
		static void SetBlock(byte* p, int value, int size) {
		  int y;
		  for (y = 0; y < size; ++y) {
			memset(p, value, size);
			p += BPS;
		...
		const int kHeaderSizeEstimate = (15 + 20 + 10);      // TODO: fix better

...
namespace NWebp.Internal
{
	class quant
	{
		const int DO_TRELLIS_I4  1
		const int DO_TRELLIS_I16 1   // not a huge gain, but ok at low bitrate.
		const int DO_TRELLIS_UV  0   // disable trellis for UV. Risky. Not worth.
		const int USE_TDISTO 1

		const int MID_ALPHA 64      // neutral value for susceptibility
		const int MIN_ALPHA 30      // lowest usable value for susceptibility
		const int MAX_ALPHA 100     // higher meaninful value for susceptibility

		const double SNS_TO_DQ 0.9     // Scaling constant between the sns value and the QP
								  // power-law modulation. Must be strictly less than 1.

		const int MULT_8B(a, b) (((a) * (b) + 128) >> 8)
		...
		static byte kBiasMatrices[3][16] = {  // [3] = [luma-ac,luma-dc,chroma]
		  { 96, 96, 96, 96,
			96, 96, 96, 96,
			96, 96, 96, 96,
			96, 96, 96, 96 },
		...
		static byte kFreqSharpening[16] = {
		  0,  30, 60, 90,
		  30, 60, 90, 90,
		  60, 90, 90, 90,
		  90, 90, 90, 90
		...
		static void SetupMatrices(VP8Encoder* enc) {
		  ...
		  int tlambda_scale =
			(enc.method_ >= 4) ? enc.config_.sns_strength
								: 0;
		  int num_segments = enc.segment_hdr_.num_segments_;
		  for (i = 0; i < num_segments; ++i) {
			VP8SegmentInfo* m = &enc.dqm_[i];
			int q = m.quant_;
			int q4, q16, quv;
		...
		const int FSTRENGTH_CUTOFF 3

		static void SetupFilterStrength(VP8Encoder* enc) {
		  int i;
		  int level0 = enc.config_.filter_strength;
		  for (i = 0; i < NUM_MB_SEGMENTS; ++i) {
			// Segments with lower quantizer will be less filtered. TODO: tune (wrt SNS)
			int level = level0 * 256 * enc.dqm_[i].quant_ / 128;
			int f = level / (256 + enc.dqm_[i].beta_);
		...
		const int MAX_DQ_UV (6)
		const int MIN_DQ_UV (-4)
		...
		void VP8SetSegmentParams(VP8Encoder* enc, float quality) {
		  ...
		  int num_segments = enc.config_.segments;
		  double amp = SNS_TO_DQ * enc.config_.sns_strength / 100. / 128.;
		  double c_base = QualityToCompression(quality);
		  for (i = 0; i < num_segments; ++i) {
			...
			// more.
			double expn = (1. - amp * enc.dqm_[i].alpha_) / 3.;
			double c = pow(c_base, expn);
			int q = (int)(127. * (1. - c));
			assert(expn > 0.);
		...
		int VP8I16ModeOffsets[4] = { I16DC16, I16TM16, I16VE16, I16HE16 };
		int VP8UVModeOffsets[4] = { C8DC8, C8TM8, C8VE8, C8HE8 };

		// Must be indexed using {B_DC_PRED . B_HU_PRED} as index
		int VP8I4ModeOffsets[NUM_BMODES] = {
		  I4DC4, I4TM4, I4VE4, I4HE4, I4RD4, I4VR4, I4LD4, I4VL4, I4HD4, I4HU4
		};

		void VP8MakeLuma16Preds(VP8EncIterator* it) {
		  VP8Encoder* enc = it.enc_;
		  byte* left = it.x_ ? enc.y_left_ : null;
		  byte* top = it.y_ ? enc.y_top_ + it.x_ * 16 : null;
		  VP8EncPredLuma16(it.yuv_p_, left, top);
		}

		void VP8MakeChroma8Preds(VP8EncIterator* it) {
		  VP8Encoder* enc = it.enc_;
		  byte* left = it.x_ ? enc.u_left_ : null;
		  byte* top = it.y_ ? enc.uv_top_ + it.x_ * 16 : null;
		...
		static ushort kWeightY[16] = {
		  38, 32, 20, 9, 32, 28, 17, 7, 20, 17, 10, 4, 9, 7, 4, 2
		};

		static ushort kWeightTrellis[16] = {
		#if !USE_TDISTO
		...
		const int MIN_DELTA 0   // how much lower level to try
		const int MAX_DELTA 1   // how much higher
		const int NUM_NODES (MIN_DELTA + 1 + MAX_DELTA)
		void NODE(n, l) { return (nodes[(n) + 1][(l) + MIN_DELTA]); }

		static void SetRDScore(int lambda, VP8ModeScore* rd) {
		...
		static int TrellisQuantizeBlock(VP8EncIterator* it,
										short in[16], short out[16],
										int ctx0, int coeff_type,
										VP8Matrix* mtx,
										int lambda) {
		  ProbaArray* last_costs = it.enc_.proba_.coeffs_[coeff_type];
		  CostArray* costs = it.enc_.proba_.level_cost_[coeff_type];
		  int first = (coeff_type == 0) ? 1 : 0;
...

		  {
			score_t cost;
			score_t max_error;
			int thresh = mtx.q_[1] * mtx.q_[1] / 4;
			int last_proba = last_costs[VP8EncBands[first]][ctx0][0];
		  ...
		  for (n = first; n <= last; ++n) {
			...
			for (m = -MIN_DELTA; m <= MAX_DELTA; ++m) {
			  ...
			  for (p = -MIN_DELTA; p <= MAX_DELTA; ++p) {
				Node* prev = &NODE(n - 1, p);
				int prev_ctx = prev.ctx;
				ushort* tcost = costs[VP8EncBands[n]][prev_ctx];
				score_t total_error = prev.error - delta_error;
		...
		static int ReconstructIntra16(VP8EncIterator* it,
									  VP8ModeScore* rd,
									  byte* yuv_out,
									  int mode) {
		  VP8Encoder* enc = it.enc_;
		  byte* ref = it.yuv_p_ + VP8I16ModeOffsets[mode];
		  byte* src = it.yuv_in_ + Y_OFF;
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int nz = 0;
		  int n;
		  short tmp[16][16], dc_tmp[16];
		...
		static int ReconstructIntra4(VP8EncIterator* it,
									 short levels[16],
									 byte* src,
									 byte* yuv_out,
									 int mode) {
		  VP8Encoder* enc = it.enc_;
		  byte* ref = it.yuv_p_ + VP8I4ModeOffsets[mode];
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int nz = 0;
		  short tmp[16];
		...
		static int ReconstructUV(VP8EncIterator* it, VP8ModeScore* rd,
								 byte* yuv_out, int mode) {
		  VP8Encoder* enc = it.enc_;
		  byte* ref = it.yuv_p_ + VP8UVModeOffsets[mode];
		  byte* src = it.yuv_in_ + U_OFF;
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int nz = 0;
		  int n;
		  short tmp[8][16];
		...
		static void SwapPtr(byte** a, byte** b) {
		  byte* tmp = *a;
		  *a = *b;
		  *b = tmp;
		}
		...
		static void PickBestIntra16(VP8EncIterator* it, VP8ModeScore* rd) {
		  VP8Encoder* enc = it.enc_;
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int lambda = dqm.lambda_i16_;
		  int tlambda = dqm.tlambda_;
		  byte* src = it.yuv_in_ + Y_OFF;
		  VP8ModeScore rd16;
		  int mode;
		  ...
		  for (mode = 0; mode < 4; ++mode) {
			byte* tmp_dst = it.yuv_out2_ + Y_OFF;  // scratch buffer
			int nz;
		...
		static ushort* GetCostModeI4(VP8EncIterator* it,
											 byte modes[16]) {
		  int preds_w = it.enc_.preds_w_;
		  int x = (it.i4_ & 3), y = it.i4_ >> 2;
		  int left = (x == 0) ? it.preds_[y * preds_w - 1] : modes[it.i4_ - 1];
		  int top = (y == 0) ? it.preds_[-preds_w + x] : modes[it.i4_ - 4];
		...
		static int PickBestIntra4(VP8EncIterator* it, VP8ModeScore* rd) {
		  VP8Encoder* enc = it.enc_;
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int lambda = dqm.lambda_i4_;
		  int tlambda = dqm.tlambda_;
		  byte* src0 = it.yuv_in_ + Y_OFF;
		  byte* best_blocks = it.yuv_out2_ + Y_OFF;
		  ...
		  do {
			...
			int mode;
			int best_mode = -1;
			byte* src = src0 + VP8Scan[it.i4_];
			ushort* mode_costs = GetCostModeI4(it, rd.modes_i4);
			byte* best_block = best_blocks + VP8Scan[it.i4_];
			byte* tmp_dst = it.yuv_p_ + I4TMP;    // scratch buffer.
		...
		static void PickBestUV(VP8EncIterator* it, VP8ModeScore* rd) {
		  VP8Encoder* enc = it.enc_;
		  VP8SegmentInfo* dqm = &enc.dqm_[it.mb_.segment_];
		  int lambda = dqm.lambda_uv_;
		  byte* src = it.yuv_in_ + U_OFF;
		  byte* tmp_dst = it.yuv_out2_ + U_OFF;  // scratch buffer
		  byte* dst0 = it.yuv_out_ + U_OFF;
		  VP8ModeScore rd_best;
		  int mode;
		...
		static void SimpleQuantize(VP8EncIterator* it, VP8ModeScore* rd) {
		  ...
		  if (i16) {
			...
			do {
			  ...
			  byte* src = it.yuv_in_ + Y_OFF + VP8Scan[it.i4_];
			  byte* dst = it.yuv_out_ + Y_OFF + VP8Scan[it.i4_];
			  VP8MakeIntra4Preds(it);

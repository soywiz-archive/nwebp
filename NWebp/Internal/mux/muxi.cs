...
namespace NWebp.Internal.mux
{
	class muxi
	{
		...
		static WEBP_INLINE uint GetLE32(const byte* const data) {
		  return data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
		}

		static WEBP_INLINE void PutLE16(byte* const data, ushort val) {
		  data[0] = (val >> 0) & 0xff;
		  data[1] = (val >> 8) & 0xff;
		}

		static WEBP_INLINE void PutLE32(byte* const data, uint val) {
		  PutLE16(data, val);
		  PutLE16(data + 2, val >> 16);
		}

		static WEBP_INLINE uint SizeWithPadding(uint chunk_size) {
		...
		static WEBP_INLINE uint ChunkDiskSize(const WebPChunk* chunk) {
		  assert(chunk->payload_size_ < MAX_CHUNK_PAYLOAD);
		  return SizeWithPadding(chunk->payload_size_);
		}

		// Total size of a list of chunks.
		uint ChunksListDiskSize(const WebPChunk* chunk_list);

		// Write out the given list of chunks into 'dst'.
		byte* ChunkListEmit(const WebPChunk* chunk_list, byte* dst);

...
namespace NWebp.Internal.webp
{
	class mux
	{
		...
		WEBP_EXTERN(WebPMux*) WebPMuxCreate(const byte* data, uint size,
											int copy_data,
		...
		//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
		//   WEBP_MUX_OK - on success.
		WEBP_EXTERN(WebPMuxError) WebPMuxAddFrame(WebPMux* const mux, uint nth,
												  const byte* data, uint size,
												  const byte* alpha_data,
												  uint alpha_size,
		...
		//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
		//   WEBP_MUX_OK - on success.
		WEBP_EXTERN(WebPMuxError) WebPMuxAddTile(WebPMux* const mux, uint nth,
												 const byte* data, uint size,
												 const byte* alpha_data,
												 uint alpha_size,
												 uint x_offset, uint y_offset,
		...
		//   WEBP_MUX_OK - on success.
		WEBP_EXTERN(WebPMuxError) WebPMuxGetTile(const WebPMux* const mux, uint nth,
												 WebPData* const image,
												 WebPData* const alpha,
												 uint* x_offset,
												 uint* y_offset);

...
namespace NWebp.Internal.webp
{
	class decode
	{
		...
		WEBP_EXTERN(int) WebPGetInfo(const byte* data, uint data_size,
		...
		WEBP_EXTERN(byte*) WebPDecodeRGB(const byte* data, uint data_size,
											int* width, int* height);

		// Same as WebPDecodeRGB, but returning RGBA data.
		WEBP_EXTERN(byte*) WebPDecodeRGBA(const byte* data, uint data_size,
											 int* width, int* height);

		// Same as WebPDecodeRGBA, but returning ARGB data.
		WEBP_EXTERN(byte*) WebPDecodeARGB(const byte* data, uint data_size,
											 int* width, int* height);

		// This variant decode to BGR instead of RGB.
		WEBP_EXTERN(byte*) WebPDecodeBGR(const byte* data, uint data_size,
											int* width, int* height);
		// This variant decodes to BGRA instead of RGBA.
		WEBP_EXTERN(byte*) WebPDecodeBGRA(const byte* data, uint data_size,
											 int* width, int* height);
		...
		WEBP_EXTERN(byte*) WebPDecodeYUV(const byte* data, uint data_size,
											int* width, int* height,
											byte** u, byte** v,
		...
		WEBP_EXTERN(byte*) WebPDecodeRGBInto(
			const byte* data, uint data_size,
			byte* output_buffer, int output_buffer_size, int output_stride);
		WEBP_EXTERN(byte*) WebPDecodeRGBAInto(
			const byte* data, uint data_size,
			byte* output_buffer, int output_buffer_size, int output_stride);
		WEBP_EXTERN(byte*) WebPDecodeARGBInto(
			const byte* data, uint data_size,
			byte* output_buffer, int output_buffer_size, int output_stride);
		// BGR variants
		WEBP_EXTERN(byte*) WebPDecodeBGRInto(
			const byte* data, uint data_size,
			byte* output_buffer, int output_buffer_size, int output_stride);
		WEBP_EXTERN(byte*) WebPDecodeBGRAInto(
			const byte* data, uint data_size,
			byte* output_buffer, int output_buffer_size, int output_stride);
		...
		WEBP_EXTERN(byte*) WebPDecodeYUVInto(
			const byte* data, uint data_size,
		...
		WEBP_EXTERN(VP8StatusCode) WebPIAppend(
			WebPIDecoder* const idec, const byte* data, uint data_size);
		...
		WEBP_EXTERN(byte*) WebPIDecGetRGB(
		...
		WEBP_EXTERN(byte*) WebPIDecGetYUV(
			const WebPIDecoder* const idec, int* last_y,
			byte** u, byte** v,
		...
		*/
		...
		WEBP_EXTERN(VP8StatusCode) WebPGetFeaturesInternal(
			const byte*, uint, WebPBitstreamFeatures* const, int);
		...
		// the error condition).
		WEBP_EXTERN(WebPIDecoder*) WebPIDecode(const byte* data, uint data_size,
											   WebPDecoderConfig* const config);

		// Non-incremental version. This version decodes the full data at once, taking
		// 'config' into account. Return decoding status (VP8_STATUS_OK if decoding
		// was successful).
		WEBP_EXTERN(VP8StatusCode) WebPDecode(const byte* data, uint data_size,
											  WebPDecoderConfig* const config);

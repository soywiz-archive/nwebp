...
namespace NWebp.Internal.webp
{
	class encode
	{
		...
		WEBP_EXTERN(size_t) WebPEncodeRGB(const byte* rgb,
										  int width, int height, int stride,
										  float quality_factor, byte** output);
		WEBP_EXTERN(size_t) WebPEncodeBGR(const byte* bgr,
										  int width, int height, int stride,
										  float quality_factor, byte** output);
		WEBP_EXTERN(size_t) WebPEncodeRGBA(const byte* rgba,
										   int width, int height, int stride,
										   float quality_factor, byte** output);
		WEBP_EXTERN(size_t) WebPEncodeBGRA(const byte* bgra,
										   int width, int height, int stride,
										   float quality_factor, byte** output);
		...
		typedef int (*WebPWriterFunction)(const byte* data, size_t data_size,
		...
		struct WebPPicture {
		  ...
		  byte *y, *u, *v;        // pointers to luma/chroma planes.
		  int y_stride, uv_stride;   // luma/chroma strides.
		  byte *a;                // pointer to the alpha plane
		  ...
		  byte* extra_info;    // if not NULL, points to an array of size
		...
		WEBP_EXTERN(int) WebPPictureImportRGB(
			WebPPicture* const picture, const byte* const rgb, int rgb_stride);
		// Same, but for RGBA buffer
		WEBP_EXTERN(int) WebPPictureImportRGBA(
			WebPPicture* const picture, const byte* const rgba, int rgba_stride);

		// Variant of the above, but taking BGR(A) input:
		WEBP_EXTERN(int) WebPPictureImportBGR(
			WebPPicture* const picture, const byte* const bgr, int bgr_stride);
		WEBP_EXTERN(int) WebPPictureImportBGRA(
			WebPPicture* const picture, const byte* const bgra, int bgra_stride);

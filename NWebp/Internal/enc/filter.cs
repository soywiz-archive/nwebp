...
namespace NWebp.Internal.enc
{
	class filter
	{
		...
		static void DoFilter(const VP8EncIterator* const it, int level) {
		  ...
		  byte* const y_dst = it->yuv_out2_ + Y_OFF;
		  byte* const u_dst = it->yuv_out2_ + U_OFF;
		  byte* const v_dst = it->yuv_out2_ + V_OFF;
		...
		static void VP8SSIMAccumulate(const byte* src1, int stride1,
		...
		static double GetMBSSIM(const byte* yuv1, const byte* yuv2) {
		  int x, y;

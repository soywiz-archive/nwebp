...
namespace NWebp.Internal
{
	...
	class analysis
	{
		...
		static void SmoothSegmentMap(VP8Encoder* enc) {
		  ...
		  int w = enc.mb_w_;
		  int h = enc.mb_h_;
		  int majority_cnt_3_x_3_grid = 5;
		  byte* tmp = (byte*)malloc(w * h * sizeof(byte));

		  if (tmp == null) return;
		  for (y = 1; y < h - 1; ++y) {
			for (x = 1; x < w - 1; ++x) {
			  int cnt[NUM_MB_SEGMENTS] = { 0 };
			  VP8MBInfo* mb = &enc.mb_info_[x + w * y];
		...
		static void SetSegmentProbas(VP8Encoder* enc) {
		  ...
		  if (enc.segment_hdr_.num_segments_ > 1) {
			byte* probas = enc.proba_.segments_;
		...
		static void SetSegmentAlphas(VP8Encoder* enc,
									 int centers[NUM_MB_SEGMENTS],
									 int mid) {
		  int nb = enc.segment_hdr_.num_segments_;
		...
		static void AssignSegments(VP8Encoder* enc, int alphas[256]) {
		  ...
		  for (k = 0; k < MAX_ITERS_K_MEANS; ++k) {     // few iters are enough
			...
			for (n = 0; n < nb; ++n) {
			  if (accum[n]) {
				int new_center = (dist_accum[n] + accum[n] / 2) / accum[n];
		  ...
		  if (nb > 1) {
			int smooth = (enc.config_.preprocessing & 1);
		...
		static int MBAnalyzeBestIntra16Mode(VP8EncIterator* it) {
		  ...
		  for (mode = 0; mode < max_mode; ++mode) {
			int alpha = VP8CollectHistogram(it.yuv_in_ + Y_OFF,
		...
		static int MBAnalyzeBestIntra4Mode(VP8EncIterator* it,
										   int best_alpha) {
		  byte modes[16];
		  int max_mode = (it.enc_.method_ >= 3) ? MAX_INTRA4_MODE : NUM_BMODES;
		  int i4_alpha = 0;
		  ...
		  do {
			...
			for (mode = 0; mode < max_mode; ++mode) {
			  int alpha = VP8CollectHistogram(src,
		...
		static int MBAnalyzeBestUVMode(VP8EncIterator* it) {
		  int best_alpha = -1;
		  int best_mode = 0;
		  int max_mode = (it.enc_.method_ >= 3) ? MAX_UV_MODE : 4;
		  int mode;
		  VP8MakeChroma8Preds(it);
		  for (mode = 0; mode < max_mode; ++mode) {
			int alpha = VP8CollectHistogram(it.yuv_in_ + U_OFF,
		...
		static void MBAnalyze(VP8EncIterator* it,
							  int alphas[256], int* uv_alpha) {

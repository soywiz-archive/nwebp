...
namespace NWebp.Internal.enc
{
	class picture
	{
		...
		static void CopyPlane(const byte* src, int src_stride,
		...
		static void RescalePlane(const byte* src,
		...
		static int Import(WebPPicture* const picture,
						  const byte* const rgb, int rgb_stride,
						  int step, int swap_rb, int import_alpha) {
		  const WebPEncCSP uv_csp = picture->colorspace & WEBP_CSP_UV_MASK;
		  int x, y;
		  const byte* const r_ptr = rgb + (swap_rb ? 2 : 0);
		  const byte* const g_ptr = rgb + 1;
		  const byte* const b_ptr = rgb + (swap_rb ? 0 : 2);
		  const int width = picture->width;
		  const int height = picture->height;
		...
		int WebPPictureImportRGBA(WebPPicture* const picture,
								  const byte* const rgba, int rgba_stride) {
		  picture->colorspace |= WEBP_CSP_ALPHA_BIT;
		  if (!WebPPictureAlloc(picture)) return 0;
		...
		#define SIZE 8
		#define SIZE2 (SIZE / 2)
		static int is_transparent_area(const byte* ptr, int stride, int size) {
		  int y, x;
		  for (y = 0; y < size; ++y) {
			for (x = 0; x < size; ++x) {
			  if (ptr[x]) {

...
namespace NWebp.Internal.enc
{
	class quant
	{
		...
		static const byte kDcTable[128] = {
		  4,     5,   6,   7,   8,   9,  10,  10,
		...
		static const ushort kAcTable[128] = {
		  4,     5,   6,   7,   8,   9,  10,  11,
		...
		static const ushort kAcTable2[128] = {
		...
		static const ushort kCoeffThresh[16] = {
		  0,  10, 20, 30,
		  10, 20, 30, 30,
		  20, 30, 30, 30,
		  30, 30, 30, 30
		...
		static const byte kBiasMatrices[3][16] = {  // [3] = [luma-ac,luma-dc,chroma]
		  { 96, 96, 96, 96,
			96, 96, 96, 96,
			96, 96, 96, 96,
			96, 96, 96, 96 },
		...
		static const byte kFreqSharpening[16] = {
		  0,  30, 60, 90,
		  30, 60, 90, 90,
		  60, 90, 90, 90,
		  90, 90, 90, 90
		...
		static const ushort kWeightY[16] = {
		  38, 32, 20, 9, 32, 28, 17, 7, 20, 17, 10, 4, 9, 7, 4, 2
		};

		static const ushort kWeightTrellis[16] = {
		#if USE_TDISTO == 0
		  ...
		  for (n = first; n <= last; ++n) {
			...
			for (m = -MIN_DELTA; m <= MAX_DELTA; ++m) {
			  ...
			  for (p = -MIN_DELTA; p <= MAX_DELTA; ++p) {
				...
				const ushort* const tcost = costs[VP8EncBands[n]][prev_ctx];
		...
		static int ReconstructIntra16(VP8EncIterator* const it,
									  ...
									  int mode) {
		  const VP8Encoder* const enc = it->enc_;
		  const byte* const ref = it->yuv_p_ + VP8I16ModeOffsets[mode];
		  const byte* const src = it->yuv_in_ + Y_OFF;
		...
		static int ReconstructIntra4(VP8EncIterator* const it,
									 ...
									 int mode) {
		  const VP8Encoder* const enc = it->enc_;
		  const byte* const ref = it->yuv_p_ + VP8I4ModeOffsets[mode];
		  const VP8SegmentInfo* const dqm = &enc->dqm_[it->mb_->segment_];
		  int nz = 0;
		  short tmp[16];
		...
		static int ReconstructUV(VP8EncIterator* const it, VP8ModeScore* const rd,
								 byte* const yuv_out, int mode) {
		  const VP8Encoder* const enc = it->enc_;
		  const byte* const ref = it->yuv_p_ + VP8UVModeOffsets[mode];
		  const byte* const src = it->yuv_in_ + U_OFF;
		  const VP8SegmentInfo* const dqm = &enc->dqm_[it->mb_->segment_];
		  int nz = 0;
		  int n;
		  short tmp[8][16];
		...
		static void PickBestIntra16(VP8EncIterator* const it, VP8ModeScore* const rd) {
		  ...
		  for (mode = 0; mode < 4; ++mode) {
			byte* const tmp_dst = it->yuv_out2_ + Y_OFF;  // scratch buffer
			int nz;
		...
		static const ushort* GetCostModeI4(VP8EncIterator* const it,
											 const byte modes[16]) {
		...
		static int PickBestIntra4(VP8EncIterator* const it, VP8ModeScore* const rd) {
		  ...
		  const int tlambda = dqm->tlambda_;
		  const byte* const src0 = it->yuv_in_ + Y_OFF;
		  byte* const best_blocks = it->yuv_out2_ + Y_OFF;
		  ...
		  do {
			...
			int mode;
			int best_mode = -1;
			const byte* const src = src0 + VP8Scan[it->i4_];
			const ushort* const mode_costs = GetCostModeI4(it, rd->modes_i4);
		...
		static void PickBestUV(VP8EncIterator* const it, VP8ModeScore* const rd) {
		  ...
		  const byte* const src = it->yuv_in_ + U_OFF;
		  byte* const tmp_dst = it->yuv_out2_ + U_OFF;  // scratch buffer
		  byte* const dst0 = it->yuv_out_ + U_OFF;
		  VP8ModeScore rd_best;
		  int mode;
		...
		static void SimpleQuantize(VP8EncIterator* const it, VP8ModeScore* const rd) {
		  ...
		  if (i16) {
			...
			do {
			  ...
			  const byte* const src = it->yuv_in_ + Y_OFF + VP8Scan[it->i4_];
			  byte* const dst = it->yuv_out_ + Y_OFF + VP8Scan[it->i4_];
			  VP8MakeIntra4Preds(it);

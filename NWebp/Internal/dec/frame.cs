...
namespace NWebp.Internal.dec
{
	public partial class Internal
	{
		...
		static const byte[] kFilterExtraRows = new byte[3] { 0, 2, 8 };
		...
		static void DoFilter(const VP8Decoder* const dec, int mb_x, int mb_y) {
			...
			byte* const y_dst = dec->cache_y_ + ctx->id_ * 16 * y_bps + mb_x * 16;
			...
			byte* const u_dst = dec->cache_u_ + ctx->id_ * 8 * uv_bps + mb_x * 8;
			byte* const v_dst = dec->cache_v_ + ctx->id_ * 8 * uv_bps + mb_x * 8;
		...
		void VP8StoreBlock(VP8Decoder* const dec) {
			...
			byte* const ydst = dec->cache_y_ + dec->mb_x_ * 16 + y_offset;
			byte* const udst = dec->cache_u_ + dec->mb_x_ * 8 + uv_offset;
			byte* const vdst = dec->cache_v_ + dec->mb_x_ * 8 + uv_offset;
			for (y = 0; y < 16; ++y) {
		...
		static int FinishRow(VP8Decoder* const dec, VP8Io* const io) {
			...
			byte* const ydst = dec->cache_y_ - ysize + y_offset;
			byte* const udst = dec->cache_u_ - uvsize + uv_offset;
			byte* const vdst = dec->cache_v_ - uvsize + uv_offset;
		...
		static int AllocateMemory(VP8Decoder* const dec) {
			...
			const uint intra_pred_mode_size = 4 * mb_w * sizeof(byte);
			const uint top_size = (16 + 8 + 8) * mb_w;
			const uint mb_info_size = (mb_w + 1) * sizeof(VP8MB);
			const uint f_info_size =
				(dec->filter_type_ > 0) ?
					mb_w * (dec->use_threads_ ? 2 : 1) * sizeof(VP8FInfo)
				: 0;
			const uint yuv_size = YUV_SIZE * sizeof(*dec->yuv_b_);
			const uint coeffs_size = 384 * sizeof(*dec->coeffs_);
			const uint cache_height = (16 * num_caches
									+ kFilterExtraRows[dec->filter_type_]) * 3 / 2;
			const uint cache_size = top_size * cache_height;
			const uint alpha_size =
				dec->alpha_data_ ? (dec->pic_hdr_.width_ * dec->pic_hdr_.height_) : 0;
			const uint needed = intra_pred_mode_size
		...
		void VP8ReconstructBlock(VP8Decoder* const dec) {
			byte* const y_dst = dec->yuv_b_ + Y_OFF;
			byte* const u_dst = dec->yuv_b_ + U_OFF;
			byte* const v_dst = dec->yuv_b_ + V_OFF;
			...
			byte* const top_y = dec->y_t_ + dec->mb_x_ * 16;
			byte* const top_u = dec->u_t_ + dec->mb_x_ * 8;
			byte* const top_v = dec->v_t_ + dec->mb_x_ * 8;
			const short* coeffs = dec->coeffs_;
			int n;
			...
			if (dec->is_i4x4_) {   // 4x4
				uint* const top_right = (uint*)(y_dst - BPS + 16);
			...
			}
			{
				...
				const short* const u_coeffs = dec->coeffs_ + 16 * 16;
				...
				const short* const v_coeffs = dec->coeffs_ + 20 * 16;

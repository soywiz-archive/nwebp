...
namespace NWebp.Internal.dec
{
	public partial class Internal
	{
		...
		static int EmitYUV(const VP8Io* const io, WebPDecParams* const p) {
		  ...
		  byte* const y_dst = buf->y + io->mb_y * buf->y_stride;
		  byte* const u_dst = buf->u + (io->mb_y >> 1) * buf->u_stride;
		  byte* const v_dst = buf->v + (io->mb_y >> 1) * buf->v_stride;
		...
		static int EmitSampledRGB(const VP8Io* const io, WebPDecParams* const p) {
		  ...
		  const byte* y_src = io->y;
		  const byte* u_src = io->u;
		  const byte* v_src = io->v;
		...
		static int EmitFancyRGB(const VP8Io* const io, WebPDecParams* const p) {
		  ...
		  const byte* cur_y = io->y;
		  const byte* cur_u = io->u;
		  const byte* cur_v = io->v;
		  const byte* top_u = p->tmp_u;
		  const byte* top_v = p->tmp_v;
		  int y = io->mb_y;
		  ...
		  if (alpha) {
			for (j = 0; j < mb_h; ++j) {
			  for (i = 0; i < mb_w; ++i) {
				...
				const uint alpha_val = clip((alpha[i] + 8) >> 4, 15);
		...
		static int Rescale(const byte* src, int src_stride,
		...
		// is consumed.
		static int Import(const byte* src, int src_stride,
						  int new_lines, WebPRescaler* const wrk) {
		  int num_lines_in = 0;
		...
		static int ExportAlphaRGBA4444(WebPDecParams* const p, int y_pos) {
		  ...
		  while (p->scaler_a.y_accum <= 0) {
			...
			for (i = 0; i < p->scaler_a.dst_width; ++i) {
			  // Fill in the alpha value (converted to 4 bits).
			  const uint alpha_val = clip((p->scaler_a.dst[i] + 8) >> 4, 15);
			  dst[2 * i] = (dst[2 * i] & 0xf0) | alpha_val;
			}
			dst += buf->stride;
			++num_lines_out;

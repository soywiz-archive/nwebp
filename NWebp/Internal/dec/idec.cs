...
namespace NWebp.Internal.dec
{
	public partial class Internal
	{
		...
		struct MemBuffer
		{
		  ...
		  const byte* part0_buf_;  // buffer to store partition #0
		}
		...
		static uint MemDataSize(const MemBuffer* mem) {
		...
		static int AppendToMemBuffer(WebPIDecoder* const idec,
									 const byte* const data, uint data_size) {
		  ...
		  if (mem->end_ + data_size > mem->buf_size_) {  // Need some free memory
			...
			const byte* const base = mem->buf_ + mem->start_;
		...
		static VP8StatusCode DecodeWebPHeaders(WebPIDecoder* const idec) {
		  const byte* data = idec->mem_.buf_ + idec->mem_.start_;
		...
		static VP8StatusCode DecodeVP8FrameHeader(WebPIDecoder* const idec) {
		  const byte* data = idec->mem_.buf_ + idec->mem_.start_;
		  const uint curr_size = MemDataSize(&idec->mem_);
		  uint bits;
		...
		static int CopyParts0Data(WebPIDecoder* idec) {
		  ...
		  const uint psize = br->buf_end_ - br->buf_;
		  MemBuffer* const mem = &idec->mem_;
		  ...
		  if (mem->mode_ == MEM_MODE_APPEND) {
			...
			byte* const part0_buf = (byte*)malloc(psize);
			if (!part0_buf) {
			  return 0;
			}
		...
		WebPIDecoder* WebPIDecode(const byte* data, uint data_size,
		...
		byte* WebPIDecGetYUV(const WebPIDecoder* const idec, int* last_y,
								byte** u, byte** v,
